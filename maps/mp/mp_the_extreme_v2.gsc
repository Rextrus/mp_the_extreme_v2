...
/*
if you remove this script, the map not not able to play anymore!
...
#include maps\mp\mp_the_extreme_v2_cameras;
...
guncache()
{
    ...
    thread gun("gun12", "deserteaglegold_mp");
...
door1()
{
    ...
    while(1)
    {
        ...
        if(player.keys >= 1 || player.vip)
        {
            ...
            if(isdefined(level.doormove) && !level.doormove)
            {
	            level.doormove = true;
...
letters()
{
    for(i=0;i<6;i++)
    {
        ent = getent("letter"+i,"targetname");
        if(!isDefined(ent))
            continue;

        ent thread lettertrig(i);
    }
}
...
lettertrig(i)
{
    while(1)
    {
        self waittill("trigger", player);
        //Lock the mutex to get a message cooldown
        if(!isDefined(level.Letter) || !isDefined(level.Letter[i]) || !level.Letter[i])
        {
            player thread LetterMessage(i);
        }
        else
...
LetterMessage(i)
{
    ...
    {
        case 0:
            iprintlnBold(self.name+" found the letter ^5A^7!");
            break;
        case 1:
            iprintlnBold(self.name+" found the letter ^5R^7!");
            break;
        case 2:
            iprintlnBold(self.name+" found the letter ^5K^7!");
            break;
        case 3:
            iprintlnBold(self.name+" found the letter ^5A^7!");
            break;
        case 4:
            iprintlnBold(self.name+" found the letter ^5N^7!");
            break;
        case 5:
            iprintlnBold(self.name+" found the letter ^5I^7!");
            break;
        default:
            what = "";
    }
...
arkani()
{
    ...
    for(;;)
    {
        ...
        level.lettercount = level.Letter[0] + level.Letter[1] + level.Letter[2] + level.Letter[3] + level.Letter[4] + level.Letter[5];

        if(level.lettercount == 6 || who.vip)
        {
        ...
        else
        {
            ...
            if(level.lettercount != 1)
                who iprintlnbold("^5"+level.lettercount+" ^7letters have been found already.");
            else
                who iprintlnbold("^5"+level.lettercount+" ^7letter has been found already.");
                wait 5;
        }
        wait 1;
    }
